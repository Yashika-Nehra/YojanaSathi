...
export default function Results(){
  ...
  return <section className="mx-auto max-w-7xl px-5 py-10 sm:px-8">
    <div className="mb-7 flex flex-col gap-5 border-b border-line pb-7 lg:flex-row lg:items-end lg:justify-between"><div><div className="card-label">02 · {t("results.label")}</div><h1 className="mt-2 font-serif text-4xl font-bold">{t("results.heading")}</h1><p className="mt-2 text-ink/70">{t("results.total",{count:data?.total??0})}</p></div><Link to="/" className="btn btn-outline w-fit"><RotateCcw size={17} aria-hidden="true"/>{t("results.edit")}</Link></div>
    <div className="mb-6 flex flex-col gap-4"><div className="flex flex-wrap border border-line bg-surface" role="tablist" aria-label={t("results.tabsLabel")}>{tabs.map(type=><button key={type} className={`tab-btn ${active===type?"tab-active":""}`} role="tab" aria-selected={active===type} onClick={()=>selectTab(type)}>{t(`types.${type}`)} <span className="ml-1 text-xs">({data?.counts?.[type]??0})</span></button>)}</div><div className="flex items-center gap-2" aria-label={t("results.areaLabel")}>{["all","rural","urban"].map(v=><button key={v} type="button" className={`filter-btn ${mode===v?"filter-active":""}`} onClick={()=>selectMode(v)}>{v==="all"?t("results.allAreas"):v==="rural"?t("form.rural"):t("form.urban")}</button>)}</div></div>
