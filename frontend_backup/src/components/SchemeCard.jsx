...
export default function SchemeCard({ scheme }) {
  ...
  return (
    <article className={`scheme-card scheme-${scheme.type}`}>
      ...
      <section className="mb-4"><h4 className="text-sm font-bold">{t("card.why")}</h4><ul className="mt-2 space-y-1 text-sm text-ink/80">{scheme.matched_reasons?.map((r,i)=><li key={i}>• {r}</li>)}</ul></section>
